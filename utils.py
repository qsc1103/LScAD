...
def vis_image_0(imgs, pred_point, gt_masks, save_path, reverse=False, points=None, boxes=None, p_label=None):
    b, c, h, w = pred_point.size()
    row_num = 1  # 固定为1

    # 将BGR格式转换为RGB
    imgs = imgs[:, [2, 1, 0], :, :].cpu().numpy().transpose(0, 2, 3, 1)  # 转换为numpy格式并调整维度顺序

    # 如果预测掩码值超出0-1范围，则使用sigmoid归一化
    if torch.max(pred_point) > 1 or torch.min(pred_point) < 0:
        pred_point = torch.sigmoid(pred_point)

    pred_point = pred_point.cpu().detach().numpy().transpose(0, 2, 3, 1)
    gt_masks = gt_masks.cpu().detach().numpy().transpose(0, 2, 3, 1)

    if reverse:
        pred_point = 1 - pred_point
        gt_masks = 1 - gt_masks

    # 创建 matplotlib 图像网格
    fig, axs = plt.subplots(4, row_num, figsize=(5, 30))  # 添加第5和第6行

    # 确保 axs 是二维数组，即使只有一列
    axs = np.array(axs).reshape(4, row_num)
    ...
    for i in range(row_num):
        ...
        # 绘制点提示生成的预测掩码
        axs[1, i].imshow(pred_point[i], cmap='jet')
        axs[1, i].set_title("Predicted Mask (Point Prompt)")
        axs[1, i].axis('off')


        # 绘制GT掩码并添加提示点和框
        axs[2, i].imshow(gt_masks[i], cmap='gray')
        axs[2, i].set_title("Ground Truth Mask")
        axs[2, i].axis('off')

        # 绘制提示点
        if points is not None:
            ps = points[i].cpu().numpy()  # 分别读取每张图的点
            for j, p in enumerate(ps):
                if p_label is not None and torch.all(p_label[i] == 1):
                    color = 'red'
                elif p_label is not None and torch.all(p_label[i] == 0):
                    color = 'green'
                else:
                    color = 'blue'

                axs[2, i].scatter(p[1], p[0], color=color, s=100, marker='*')  # 使用五角星形状绘制点
        ...
        # 在第六个子图中绘制原始图像和文本提示生成的预测掩码叠加图
        axs[3, i].imshow(imgs[i])  # 显示原始图像
        axs[3, i].imshow(pred_point[i], cmap='jet', alpha=0.5)  # 以透明度为0.5叠加显示预测掩码
        axs[3, i].set_title("Overlayed Image with Text Prompt Mask")
        axs[3, i].axis('off')
...
def vis_image(imgs, pred_masks, gt_masks, anomaly_map, binary_mask, save_path, reverse=False, points=None, boxes=None, p_label=None):
    b, c, h, w = pred_masks.size()
    row_num = 1  # 一行展示1张图像

    # 转换图像格式 BGR -> RGB
    imgs = imgs[:, [2, 1, 0], :, :].cpu().numpy().transpose(0, 2, 3, 1)

    # 如果预测掩码值大于1或小于0，则使用sigmoid归一化
    if torch.max(pred_masks) > 1 or torch.min(pred_masks) < 0:
        pred_masks = torch.sigmoid(pred_masks)

    pred_masks = pred_masks.cpu().detach().numpy().transpose(0, 2, 3, 1)
    gt_masks = gt_masks.cpu().detach().numpy().transpose(0, 2, 3, 1)
    ...
    # 创建一个包含7行的子图，每行展示一张图像
    fig, axs = plt.subplots(7, row_num, figsize=(5, 35))
    axs = np.array(axs).reshape(7, row_num)
    ...
    for i in range(row_num):
        ...
        # 显示Anomaly Map，并在标题中显示最大值和最小值
        axs[1, i].imshow(anomaly_map, cmap='jet')
        axs[1, i].set_title(f"Anomaly Map Max: {anomaly_map.max():.2f}, Min: {anomaly_map.min():.2f}")
        axs[1, i].axis('off')

        # 显示二值化掩码
        axs[2, i].imshow(binary_mask, cmap='gray')
        axs[2, i].set_title('Binary Mask')
        axs[2, i].axis('off')

        # 在Anomaly Map上显示points，使用p_label来决定颜色
        if points is not None and p_label is not None:
            coords, labels = points  # 从元组中解包
            coords = coords[0].cpu().numpy()  # 将坐标从 GPU 移到 CPU，并转换为 numpy
            labels = labels[0].cpu().numpy()  # 将标签从 GPU 移到 CPU，并转换为 numpy
            
            for j, point in enumerate(coords):
                color = 'red' if labels[j] == 1 else 'green'  # 根据标签选择颜色
                axs[2, i].scatter(point[1], point[0], color=color, s=100, marker='*')  # 绘制点

        # 如果有bounding boxes，画出红框
        if boxes is not None:
        ...
        # 显示基于 point 提示的预测掩码
        axs[3, i].imshow(pred_masks[i], cmap='jet')
        axs[3, i].set_title('Predicted Mask (Point)')
        axs[3, i].axis('off')

        # 显示 Ground Truth 掩码
        axs[4, i].imshow(gt_masks[i], cmap='gray')
        axs[4, i].set_title('Ground Truth Mask')
        axs[4, i].axis('off')

        # 在原始图像上覆盖显示 Anomaly Map
        axs[5, i].imshow(imgs[i])  # 显示原始图像
        axs[5, i].imshow(anomaly_map, cmap='jet', alpha=0.5, extent=(0, imgs[i].shape[1], imgs[i].shape[0], 0))
        axs[5, i].set_title('Overlay Image with Anomaly Map')
        axs[5, i].axis('off')

        # 在原始图像上叠加显示预测掩码
        axs[6, i].imshow(imgs[i])
        axs[6, i].imshow(pred_masks[i], cmap='jet', alpha=0.5, extent=(0, imgs[i].shape[1], imgs[i].shape[0], 0))
        axs[6, i].set_title('Overlayed Image with Predicted Mask')
        axs[6, i].axis('off')
...
def apply_colormap(tensor):
    ...
    将张量应用Jet颜色映射
    """
    tensor = tensor.squeeze(1).detach().cpu().numpy()  # 在转换为NumPy数组之前调用detach
    colored = np.stack([cm.jet(tensor[i])[..., :3] for i in range(tensor.shape[0])], axis=0)
    return torch.tensor(colored).permute(0, 3, 1, 2).to(dtype=torch.float32)
...
def eval_seg_auroc(preds, true_masks):
    '''
    计算所有样本的像素级别的AUROC值。

    参数:
    - preds: 模型的预测输出列表，每个元素形状为 [b, h, w]。
    - true_masks: 真实掩码图像列表，每个元素形状为 [b, h, w]。
    ...
    # 将所有样本展平成一维向量
    preds_flat = np.concatenate([pred.cpu().detach().numpy().flatten() for pred in preds])
    true_masks_flat = np.concatenate([true_mask.cpu().detach().numpy().flatten() for true_mask in true_masks])

    # 检查 true_masks_flat 中是否包含至少两个类别
    if len(np.unique(true_masks_flat)) > 1:
        # 计算整体AUROC值
        auroc = roc_auc_score(true_masks_flat, preds_flat)
    else:
        # 如果只有一个类别，返回一个默认值或提示
        print(f"Warning: Only one class present in y_true across all samples. AUROC calculation is not possible.")
        auroc = None
...
def random_click(mask, point_labels=1, num_points=10):
    """
    生成指定数量的点提示。

    参数:
    - mask: 输入的二值掩码图像。
    - point_labels: 点的标签，默认为1（前景）。
    - num_points: 生成的点的数量，默认为10。

    返回:
    - point_labels: 点的标签数组。
    - points: 生成的点坐标数组，形状为(num_points, 2)。
    """
    ...
    if max_label == 0:
        ...
        points = np.zeros((num_points, 2), dtype=int)  # 如果全黑，返回全零点
    else:
        # 获取掩码中值为 max_label 的所有像素位置
        indices = np.argwhere(mask == max_label)
        # 随机选择 num_points 个位置
        if len(indices) >= num_points:
            selected_indices = indices[np.random.choice(len(indices), num_points, replace=False)]
        else:
            selected_indices = indices[np.random.choice(len(indices), num_points, replace=True)]

        # 构造返回的点标签数组和坐标数组
        point_labels = np.full(num_points, point_labels, dtype=int)
...
def random_click_black(mask, point_labels=0, num_points=10):
    """
    在全黑掩码图像中随机生成指定数量的点提示。
...
def find_disconnected_bounding_boxes(mask: np.ndarray):
    # 如果输入mask是三维的，首先将其转换为二维
    if mask.ndim == 3:
...
def find_largest_bounding_box(mask: np.ndarray):
    # 如果输入mask是三维的，首先将其转换为二维
    if mask.ndim == 3:
        mask = np.squeeze(mask, axis=0)
    ...
    largest_box = [0, 0, mask.shape[1], mask.shape[0]]  # 初始化为整个图像的边界框
    max_area = 0  # 记录最大面积

    for contour in contours:
        x, y, w, h = cv2.boundingRect(contour)
        area = w * h  # 计算当前轮廓的面积
        if area > max_area:  # 如果当前轮廓面积比最大面积大，更新最大边界框
            max_area = area
...
def random_box(mask):
    # 找到掩码中的最大值及其位置
    max_value = np.max(mask, axis=0)
    indices = np.argwhere(mask == max_value)

    if len(indices) == 0:
        raise ValueError("The mask is empty, no non-zero elements found.")
